...
class Allocator {
public:
  __declspec(dllexport) static uint8_t *allocate(uint32_t PageCount) noexcept;

  __declspec(dllexport) static uint8_t *resize(uint8_t *Pointer, uint32_t OldPageCount,
                         uint32_t NewPageCount) noexcept;

  __declspec(dllexport) static void release(uint8_t *Pointer, uint32_t PageCount) noexcept;
