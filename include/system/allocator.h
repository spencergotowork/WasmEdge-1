...
class Allocator {
public:
  WASMEDGE_CAPI_EXPORT static uint8_t *allocate(uint32_t PageCount) noexcept;

  WASMEDGE_CAPI_EXPORT static uint8_t *resize(uint8_t *Pointer, uint32_t OldPageCount,
                         uint32_t NewPageCount) noexcept;

  WASMEDGE_CAPI_EXPORT static void release(uint8_t *Pointer, uint32_t PageCount) noexcept;
